...
reactNativeFfjavascript.Scalar.e("73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001", 16);
reactNativeFfjavascript.Scalar.e("21888242871839275222246405745257275088548364400416034343698204186575808495617");
...
async function groth16Prove(zkeyFileName, witnessFileName, logger) {
    ...
    const zkBuff = await get$1(zkeyFileName).then( function(res) {
        console.log(res);
        return res
    }).then(function (ab) {
    ...
    const {fd: fdZKey, sections: sectionsZKey} = await binFileUtils__namespace.readBinFile(zkFile, "zkey", 2, 1<<25, 1<<23);
...
async function wtnsCalculate(input, wasmFileName, wtnsFileName, options) {
    ...
    const fdWasm = await fastFile__namespace.readExisting(o);
...
async function r1csInfo(r1csName, logger) {
    ...
    if (reactNativeFfjavascript.Scalar.eq(cir.prime, bn128r)) {
        if (logger) logger.info("Curve: bn-128");
    } else if (reactNativeFfjavascript.Scalar.eq(cir.prime, bls12381r)) {
...
/*
    ...
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/
const {stringifyBigInts: stringifyBigInts$1} = reactNativeFfjavascript.utils;
